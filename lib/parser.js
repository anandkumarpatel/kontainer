...
class Parser {
  parse (input) {
    ...
    const boundMounts = this._filterBoundMounts(mounts)

    if (this._isMountsBound(boundMounts)) {
      this._addMounts(out, name, boundMounts)
    }

    this._addDeployments(out, name, ports, envs, image, command, args, boundMounts)

    return out
  }

  _isMountsBound (mounts) {
    return !!mounts.length
  }

  _filterBoundMounts (mounts) {
    return mounts.filter(this._isMountBinded)
  }
